...
public class AbstractStateAspect {
  ...
  public static AbstractState on(final AbstractState _self, final String event) {
    ...
    if (stepManager != null) {
    	stepManager.executeStep(_self,command,"AbstractState","on");
    } else {
    	command.execute();
    }
  ...
  public static void onEnter(final AbstractState _self) {
    ...
    fr.inria.diverse.k3.al.annotationprocessor.stepmanager.IStepManager stepManager = fr.inria.diverse.k3.al.annotationprocessor.stepmanager.StepManagerRegistry.getInstance().findStepManager(_self);
    if (stepManager != null) {
    	stepManager.executeStep(_self,command,"AbstractState","onEnter");
    } else {
    	command.execute();
    }
    ;;
  ...
  public static void onExit(final AbstractState _self) {
    ...
    fr.inria.diverse.k3.al.annotationprocessor.stepmanager.IStepManager stepManager = fr.inria.diverse.k3.al.annotationprocessor.stepmanager.StepManagerRegistry.getInstance().findStepManager(_self);
    if (stepManager != null) {
    	stepManager.executeStep(_self,command,"AbstractState","onExit");
    } else {
    	command.execute();
    }
    ;;
